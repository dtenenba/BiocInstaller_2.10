    ...
    if (vers > "2.13" && biocVers > "2.8") {
        ...
        if (!suppressWarnings(require("BiocInstaller", quietly=TRUE))) {
            ...
            if (!"package:utils" %in% search()) {
                ...
                ## example value:
                ## export BIOCINSTALLER_TEST_REPOS=localhost:3000/BiocInstallerTestRepos
                if (nchar(Sys.getenv("BIOCINSTALLER_TEST_REPOS")))
                {
                    a["BioCsoft", "URL"] <- sub("bioconductor.org",
                        Sys.getenv("BIOCINSTALLER_TEST_REPOS"),
                        a["BioCsoft", "URL"])
                    a["BioCsoft", "URL"] <- sub("www.", "",
                        a["BioCsoft", "URL"])
                    print(paste("test repos url =", a["BioCsoft", "URL"]))
